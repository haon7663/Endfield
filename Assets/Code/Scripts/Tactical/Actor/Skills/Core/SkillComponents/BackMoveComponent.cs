...
public class BackMoveComponent : SkillComponent
{
    public override void Execute(SkillComponentInfo info)
    {
        var movement = info.user.Movement;
        var task = new Task(movement.OnMove(CalculateDistance(info) * info.dirX * -1));
        task.Start();
    }

    public override void Print(SkillComponentInfo info)
    {
        var additionalDistance = CalculateDistance(info) * info.dirX * -1;
